...
export const ReviewProvider = ({ children }) => {
    ...
    const createReview = async (text, rating, propertyId) => {
        ...
        try {
            const response = await axios.post(`${config.API_BASE_URL}/api/reviews`, { text, rating, propertyId }, {
                headers: {
    ...
    const getReviews = async (propertyId) => {
        ...
        try {
            const response = await axios.get(`${config.API_BASE_URL}/api/reviews/${propertyId}`, {
                headers: {
                    "Authorization": `Bearer ${token}`
                }
            });
