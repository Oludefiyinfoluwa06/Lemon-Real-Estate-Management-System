...
const CustomButton = ({ onPress, text, variant = "primary", disabled = false }) => (
    <TouchableOpacity
        onPress={onPress}
        disabled={disabled}
        className={`
...
const VerifyId = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${config.API_BASE_URL}/api/user/id/verify`, formData, {
                headers: {
            ...
            Alert.alert('Error', error.response.data.message || 'An error occurred while verifying the ID.');
        } finally {
            setLoading(false);
        }
    };
        ...
        <SafeAreaView className="flex-1 bg-darkUmber-dark p-6">
            ...
            {user.isIdVerified ? (
                <View className="flex-1 flex-col items-center justify-center">
                    ...
                    <Text className="text-white font-rregular text-xl">Your ID has been verified successfully</Text>
                </View>
            ) : (
                <View>
                        ...
                        <View className="bg-frenchGray-light mb-4 rounded-lg w-full font-regular">
                            ...
                            >
                                ...
                                {documentTypes.map((doc) => (
                                    <Picker.Item key={doc.code} label={doc.label} value={doc.code} />
                                ))}
                            </Picker>
                        </View>
