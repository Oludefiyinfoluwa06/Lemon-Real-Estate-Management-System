...
const ProfilePictureUpload = () => {
    ...
    const { authLoading, authError, authMessage, setAuthError, setAuthMessage, uploadProfilePicture } = useAuth();
        ...
        <ScrollView showsVerticalScrollIndicator={false} className='h-full bg-darkUmber-light'>
            <SafeAreaView className="min-h-screen items-center justify-center p-[20px]">
                ...
                <TouchableOpacity
                    ...
                    onPress={pickImage}
                >
                    {image === null ? <Ionicons name="cloud-upload-outline" size={30} color={"#CCCCCC"} /> : <Image source={{uri: image}} className="w-full h-full" resizeMode='contain' />}
                </TouchableOpacity>
